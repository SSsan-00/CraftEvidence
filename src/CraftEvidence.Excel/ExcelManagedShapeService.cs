...
/// <summary>Inspects and mutates only the single CraftEvidence image selected in Excel.</summary>
public sealed class ExcelManagedShapeService
{
    ...
    WorkbookIdentity identity)
  {
    ...
    try
    {
      ...
      {
        return ManagedShapeSelectionResult.Failed("ExcelでCraftEvidence画像を1つ選択してください。");
      }

      var count = Convert.ToInt32(GetRequiredProperty(shapeRange, "Count"), CultureInfo.InvariantCulture);
      if (count != 1)
      {
        return ManagedShapeSelectionResult.Failed("複数選択では変更できません。CraftEvidence画像を1つだけ選択してください。");
      }
    ...
    string outputPath)
  {
    ...
    {
      return ManagedShapeExportResult.Failed($"管理画像の一時退避に失敗しました (0x{GetAutomationHResult(exception):X8})。");
    }
    finally
    {
  ...
  {
    target = null!;
    reason = "選択対象はCraftEvidence管理画像ではありません。";
