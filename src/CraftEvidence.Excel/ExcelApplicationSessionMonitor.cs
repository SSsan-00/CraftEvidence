...
public sealed class ExcelApplicationSessionMonitor : IDisposable
{
    ...
    try
    {
      ...
      while (monikerEnumerator.Next(1, monikers, IntPtr.Zero) == 0)
      {
        ...
        try
        {
          ...
          var sink = new AppEventsSink((object workbook, ref bool cancel) =>
          {
            OnWorkbookBeforeClose(candidateProcessId, workbook);
          });
  ...
  public sealed class AppEventsSink(WorkbookBeforeCloseHandler workbookBeforeClose) : IExcelAppEvents
  {
