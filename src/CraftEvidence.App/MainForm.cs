...
public sealed class MainForm : Form
{
  ...
  private void InitializeUi()
  {
    ...
    var title = new Label
    {
      ...
      Text = "Excelエビデンス画像支援 — 完成仕様レビュー版",
    };
    layout.Controls.Add(title, 0, 0);
    layout.SetColumnSpan(title, 3);

    layout.Controls.Add(CreateLabel("Workbook"), 0, 1);
    ...
    layout.Controls.Add(refreshButton, 2, 1);

    layout.Controls.Add(CreateLabel("Side"), 0, 2);
    ...
    layout.Controls.Add(CreateLabel("Sheet"), 0, 3);
    worksheetNameBox.Dock = DockStyle.Fill;
    worksheetNameBox.Text = "ActiveSheet";
    worksheetNameBox.PlaceholderText = "ActiveSheet またはシート名";
    layout.Controls.Add(worksheetNameBox, 1, 3);
    layout.SetColumnSpan(worksheetNameBox, 2);

    layout.Controls.Add(CreateLabel("Rows"), 0, 4);
    ...
    rowActions.Controls.Add(new Label { AutoSize = true, Text = "挿入数", Margin = new Padding(0, 7, 4, 0) });
    insertRowCountBox.Minimum = 1;
    insertRowCountBox.Maximum = 1000;
    insertRowCountBox.Value = 1;
    insertRowCountBox.Width = 58;
    insertRowsButton.Text = "上に行を挿入";
    insertRowsButton.AutoSize = true;
    insertRowsButton.Click += async (_, _) => await InsertRowsAsync();
    rowActions.Controls.Add(insertRowCountBox);
    rowActions.Controls.Add(insertRowsButton);
    rowActions.Controls.Add(new Label { AutoSize = true, Text = "削除 Case", Margin = new Padding(12, 7, 4, 0) });
    deleteCaseStartBox.Width = 58;
    deleteCaseStartBox.PlaceholderText = "開始";
    deleteCaseEndBox.Width = 58;
    deleteCaseEndBox.PlaceholderText = "終了";
    deleteRowsButton.Text = "安全な末尾行を削除";
    deleteRowsButton.AutoSize = true;
    deleteRowsButton.Click += async (_, _) => await DeleteTrailingRowsAsync();
    rowActions.Controls.Add(deleteCaseStartBox);
    rowActions.Controls.Add(deleteCaseEndBox);
    rowActions.Controls.Add(deleteRowsButton);
    layout.Controls.Add(rowActions, 1, 4);
    layout.SetColumnSpan(rowActions, 2);
    ...
    layout.Controls.Add(CreateLabel("Managed image"), 0, 6);
    var managedActions = new FlowLayoutPanel { Dock = DockStyle.Fill, AutoSize = true };
    replaceImageButton.Text = "選択画像を差し替え";
    replaceImageButton.AutoSize = true;
    replaceImageButton.Click += async (_, _) => await ReplaceSelectedImageAsync();
    deleteImageButton.Text = "選択画像を削除";
    deleteImageButton.AutoSize = true;
    deleteImageButton.Click += async (_, _) => await DeleteSelectedImageAsync();
    managedActions.Controls.Add(replaceImageButton);
    managedActions.Controls.Add(deleteImageButton);
    layout.Controls.Add(managedActions, 1, 6);
    layout.SetColumnSpan(managedActions, 2);

    layout.Controls.Add(CreateLabel("Capture"), 0, 7);
    ...
    var safetyNotice = new Label
    {
      AutoSize = true,
      ForeColor = Color.DarkGoldenrod,
      Text = "完成仕様レビュー版: 自動配置・画像編集・管理画像操作を実行します。自動保存は行いません。",
    };
    layout.Controls.Add(safetyNotice, 0, 8);
    layout.SetColumnSpan(safetyNotice, 3);

    layout.Controls.Add(CreateLabel("Status"), 0, 9);
    statusLabel.AutoSize = true;
    statusLabel.Text = "Ready";
    layout.Controls.Add(statusLabel, 1, 9);
    ...
    layout.Controls.Add(settingsButton, 1, 10);
    layout.Controls.Add(closeButton, 2, 10);
  ...
  private async Task PlaceImageFilesAutomaticallyAsync()
  {
    ...
    try
    {
      ...
      foreach (var sourcePath in dialog.FileNames)
      {
        ...
        var path = Path.Combine(Path.GetTempPath(), "CraftEvidence", $"batch-{Guid.NewGuid():N}.png");
  ...
  private async Task NavigateCaseAsync(CaseNavigationDirection direction)
  {
    ...
    try
    {
      ...
      var result = await StaTask.Run(() => caseNavigationService.Navigate(workbook, worksheetName, direction));
      SetStatus(result.Message);
    }
    catch (Exception exception) when (exception is not OutOfMemoryException)
    {
  ...
  private async Task ReplaceSelectedImageAsync()
  {
    ...
    try
    {
      ...
      var imagePath = Path.Combine(Path.GetTempPath(), "CraftEvidence", $"replace-{Guid.NewGuid():N}.png");
      var originalPath = Path.Combine(Path.GetTempPath(), "CraftEvidence", $"replace-original-{Guid.NewGuid():N}.png");
      try
      {
  ...
  private async Task DeleteSelectedImageAsync()
  {
    ...
    try
    {
      ...
      var originalPath = Path.Combine(Path.GetTempPath(), "CraftEvidence", $"delete-original-{Guid.NewGuid():N}.png");
      try
      {
        var exported = await ExportManagedShapePreservingClipboardAsync(
          workbook,
    ...
    double horizontalMarginPoints)
  {
    ...
    AddHistory(new HistoryEntry(
      ...
      async () =>
      {
        var imagePath = Path.Combine(Path.GetTempPath(), "CraftEvidence", $"redo-{Guid.NewGuid():N}.png");
        try
        {
          Directory.CreateDirectory(Path.GetDirectoryName(imagePath)!);
    ...
    RowDeletionSnapshot? cleanupSnapshot)
  {
    ...
    {
      var imagePath = Path.Combine(Path.GetTempPath(), "CraftEvidence", $"history-auto-{Guid.NewGuid():N}.png");
      try
      {
        Directory.CreateDirectory(Path.GetDirectoryName(imagePath)!);
    ...
    RowDeletionSnapshot? cleanupSnapshot)
  {
    ...
    AddHistory(new HistoryEntry(
      ...
      async () =>
      {
        ...
        var path = Path.Combine(Path.GetTempPath(), "CraftEvidence", $"undo-delete-{Guid.NewGuid():N}.png");
        try
        {
          Directory.CreateDirectory(Path.GetDirectoryName(path)!);
      ...
      async () =>
      {
        ...
        if (cleanupSnapshot is not null &&
          !await RunRowHistoryOperationAsync(() => rowMutationService.DeleteRestoredRows(workbook, cleanupSnapshot)))
        {
          var path = Path.Combine(Path.GetTempPath(), "CraftEvidence", $"compensate-delete-{Guid.NewGuid():N}.png");
          try
          {
            Directory.CreateDirectory(Path.GetDirectoryName(path)!);
            await File.WriteAllBytesAsync(path, originalPng);
  ...
  {
    var path = Path.Combine(Path.GetTempPath(), "CraftEvidence", $"history-replace-{Guid.NewGuid():N}.png");
    try
    {
      Directory.CreateDirectory(Path.GetDirectoryName(path)!);
  ...
  private void AddRowInsertionHistory(
    ...
    AddHistory(new HistoryEntry(
      ...
      () => RunRowHistoryOperationAsync(() => rowMutationService.InsertRows(
        workbook,
        worksheetName,
        new RowInsertion(startRow, count, "Redo CraftEvidence row insertion.")))));
  ...
  private void AddRowDeletionHistory(
    ...
    AddHistory(snapshot is null
      ...
      () => RunRowHistoryOperationAsync(() => rowMutationService.InsertRows(
        workbook,
        worksheetName,
        new RowInsertion(startRow, count, "Undo CraftEvidence row deletion."))),
      () => RunRowHistoryOperationAsync(() => rowMutationService.DeleteRowsIfSafe(
        workbook,
        worksheetName,
        startRow,
