...
public sealed class PlacementPlanner(ImageSizingService imageSizingService)
{
  ...
  public PlacementPlan Plan(PlacementRequest request)
  {
    ...
    var activeRowIsFree = request.ActiveRow >= request.Layout.StartRow &&
    ...
    {
      startRow = request.Layout.StartRow;
      mode = PlacementMode.CaseStart;
      reason = "The selected side has no existing content.";
    }
    else
    {
      startRow = ResolveTailStart(relevantContents, request.ImageGapRows);
    }
    ...
    return new PlacementPlan(
      ...
      fittedImage,
      insertions,
      new CellReference(startRow, sideColumns.FirstColumn),
      reason);
  }
