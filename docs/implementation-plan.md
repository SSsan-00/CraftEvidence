...
- 「そのまま貼る」「編集して貼る」「末尾へ配置」「キャンセル」を提供する。
...
- ActiveCell が Case 中央の空白を指す場合はその位置を優先し、空間不足ならプレビュー表示後に行を追加する。ユーザーは末尾配置へ変更できる。
- 編集機能として赤枠、矢印、テキスト、モザイク、トリミング、Undo、Redo、元画像へ戻すを提供する。
...
- 虫食いが狭い場合は不足行をその位置へ追加する計画を作るが、プレビューで「中央へ N 行追加」を明示し、末尾配置へ切り替え可能にする。
...
| 狭い虫食い位置 | 行を追加して挿入。ただしプレビューで明示し、末尾へ変更可能 |
| 左右余白 | 6pt ずつ。設定変更可能 |
