...
- New/Oldはユーザーが切り替えでき、画像追加後は選択した配置後モードに従って次の空きSide／Caseへ進む。
- `Win + Shift + S` 後の Clipboard 画像を検知し、Excel を変更する前にプレビューする。
- 「編集せず配置」「編集して配置」「キャンセル」を提供し、配置方法は自動判定する。
- キャプチャまたはClipboardで承認した単一画像を配置する。
- 画像同士の間には実Excel行で最低2行分の非画像帯を確保する。New/OldがそろったCaseは末尾2行を残して安全な空行を整理する。
- 横幅優先で縮小し、小さい画像は拡大しない。左右余白は初期値 6pt とする。
- 高さ不足時は Worksheet の行全体を追加する。
- Case 末尾の連続した不要行だけを安全条件付きで自動削除する。
- ActiveCellは初期のSheet／Case／Side候補にだけ使用し、配置は画面で確定したCaseの先頭基準位置を優先する。
...
- 配置開始位置はCase番号行の2行下、対象Side先頭の1列右とする。ActiveCellの空白位置は配置候補にしない。
- Case先頭基準位置の空きが不足する場合は、安全な末尾位置または必要な行追加を計画する。
- New/Old双方に管理画像がある場合は、両Sideの最下端の最大値から2行を残して安全な末尾空行だけを削除する。
...
- 次Caseへ侵入せず、通常は末尾4行、New/Old完成時の自動整理では末尾2行を下回らない範囲に限定する。
...
- 0/1画像、最低2行の非画像帯、帯内の補足文、完成Caseの2行余白、大小/縦長/横長画像をテストできる。
- ActiveCellとCase先頭が異なる場合、末尾行追加、末尾削除安全条件をテストできる。
...
- 書式/罫線再構築
- Case先頭基準の単一画像配置
- 同一Case反対Side／次Caseへの空きブロック遷移

完了条件:

- New/OldがそろったCaseは、長い側を基準に末尾2行を維持する。
...
- Placement: 0/1、Case先頭基準、2行の非画像帯、帯内補足文、帯外補足文、New/Oldの高さ差
- Context: ActiveCellと設定Caseの不一致、反対Side／次Caseへの遷移
...
- 単一画像、最低2行の非画像帯、完成Caseの2行末尾、配置後遷移、行増減、差し替え、削除、直前取消が仕様どおり動く。
